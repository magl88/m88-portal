...
export class SessionService {
	...
	public async login(dto: LoginRequest, ip: string, userAgent: string) {
		...
		if (!isValidPassword) throw new NotFoundException("Неверный логин или пароль");

		const mfa = await this.prismaService.multiFactorAuthentication.findUnique({
			where: {
				userId: user.id,
			},
			include: {
		...
		if (mfa && mfa.recoveryCodes.length > 0) {
			...
			if (mfa.totp?.status === TotpStatus.ENABLED) allowedMethods.push("Totp");
			if (mfa.recoveryCodes.length > 0) allowedMethods.push("Recovery");

			const ticket = await this.redisService.createMfaTicket(user.id, allowedMethods);
	...
	public async loginAdmin(dto: LoginRequest, ip: string, userAgent: string) {
		...
		if (mfa && mfa.recoveryCodes.length > 0) {
			...
			const ticket = await this.redisService.createMfaTicket(admin.id, allowedMethods);

			return ticket;
		}

		const session = await this.redisService.createSession(admin, ip, userAgent);
	...
	public async cleanupSessions() {
		...
		for (const userSession of userSessions) {
			...
			for (const sessionId in sessions) {
				const existSession = await this.redisService.exists(`sessions:${sessionId}`);
