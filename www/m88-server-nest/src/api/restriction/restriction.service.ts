...
export class RestrictionService {
	...
	public async create(dto: CreateRestrictionRequest) {
		...
		const activeRestriction = await this.prismaService.restriction.findFirst({
			where: {
				userId: user.id,
				status: RestrictionStatus.ACTIVE,
			},
		...
		if (activeRestriction)
			throw new ConflictException("User already has an active restriction");
		...
		await this.mailService.sendRestrictionEmail(user, restriction, violations);
	...
	public async cancel(userId: string) {
		...
		if (!restriction)
			throw new NotFoundException("No active restriction found for this user");
	...
	public async expireRestrictionsDaily() {
		...
		if (!restrictions.length) {
			...
			this.logger.log(`✅ Found ${restrictions.length} active restrictions to expire`);

			for (const restriction of restrictions) {
				this.logger.log(`🔄 Expiring restriction for user: ${restriction.user.id}`);
				...
				const violations = await this.getViolationsCount(restriction.user.id);

				await this.mailService.sendRestrictionLiftedEmail(restriction.user, violations);

				this.logger.log(`✅ Restriction expired for user: ${restriction.user.id}`);
			}
		}
