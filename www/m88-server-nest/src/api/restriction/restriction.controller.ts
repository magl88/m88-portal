...
export class RestrictionController {
	public constructor(private readonly restrictionService: RestrictionService) {}

	@ApiOperation({
		summary: "Get Active User Ban",
		description: "Retrieve information about the current active ban of the user, if any",
	})
	@ApiOkResponse({
	...
	@ApiOperation({
		...
		description: "Restriction a user for a specific reason with optional expiration.",
	})
	@ApiOkResponse({
		type: Boolean,
	})
